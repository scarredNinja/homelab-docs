...
| Plex                    | Worker-Media | 50      | Local               | Local                | Config on ocal transcodes/cache local                      | node.labels.type == media  | rpool/docker-data/plex                                                                         | NA      |
| Sonarr / Radarr         | Worker-Media | 50      | NFS                 | Local                | Downloads can go to NFS media shares; temp local if needed | node.labels.type == media  | rpool/docker-data/sonarr<br>rpool/docker-data/radarr                                           | NA      |
| Transmission / Prowlarr | Worker-Media | 50      | NFS                 | Local                | Downloads stored on NFS media shares; temp local           | node.labels.type == media  | rpool/docker-data/transmission<br>rpool/docker-data/prowlarr                                   | NA      |
| Home Assistant          | Controller   | 60      | NFS                 | Local                | Needs access to IoT VLAN 20                                | node.labels.nas == nfs     | rpool/docker-data/homeassistant                                                                | NA      |
| UniFi Controller        | Controller   | 60      | NFS                 | Local                | Network controller, portable config                        | node.labels.nas == nfs     | rpool/docker-data/uniFi                                                                        | NA      |
| Grafana                 | Manager      | 60      | NFS                 | Local                | Dashboards, metrics                                        | TBC                        | rpool/docker-data/grafana                                                                      | Running |
...
| Tautulli                | Worker-Media | 50      | NFS                 | Local                | Optional monitoring for Plex                               | node.labels.type == media  | rpool/docker-data/tautulli                                                                     | NA      |
