...
- [x] Deploy Traefik stack via Portainer UI
- [x] Verify both entrypoints active (`:443` websecure, `:8443` internal)
- [x] Check Cloudflare DNS challenge completes and cert is issued
- [x] Verify Portainer accessible via `portainer.home.purvishome.com` through Traefik internal entrypoint
- [x] **Check `02-provision-vm.sh` worker token handling** — confirm script reads fresh token correctly, not a stale cached value. This caused traefik-dmz-01 to join as manager.
